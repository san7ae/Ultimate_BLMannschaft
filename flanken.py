...
writer.writerow(["Name", "Verein", "Position", "Einsatz", "SpielMinuten", "FlankenGeblockt", "GesamteFlanken"])
...
for element in player_list:
    try:
        ...
        flankenGeblockt = float(flankenGeblocktStr)

        gesamtFlankenStr = element.findAll("td", {"class": "text-right"})[3].text
        gesamtFlanken = float(gesamtFlankenStr)
