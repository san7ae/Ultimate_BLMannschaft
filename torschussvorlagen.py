...
page = "https://www.ligainsider.de/bundesliga/ranking-top-10/torschussvorlagen/"
pageTree = requests.get(page, headers=headers)
pageSoup = bs4.BeautifulSoup(pageTree.content, 'html.parser')

# Write to CSV data
outfile = open('torschussvorlagen.csv','w', encoding="utf-8", newline='')
writer = csv.writer(outfile, delimiter=",")
writer.writerow(["Name", "Verein", "Position", "Einsatz", "SpielMinuten", "TorschussVorlagen"])
...
for element in player_list:
    try:
        ...
        torschussvorlagenStr = element.findAll("td", {"class": "text-right"})[2].text
        torschussvorlagen = float(torschussvorlagenStr)
