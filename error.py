...
page = "https://www.ligainsider.de/bundesliga/ranking-top-10/error/"
pageTree = requests.get(page, headers=headers)
pageSoup = bs4.BeautifulSoup(pageTree.content, 'html.parser')

# Write to CSV data
outfile = open('error.csv','w', encoding="utf-8", newline='')
writer = csv.writer(outfile, delimiter=" ")
writer.writerow(["Name", "Verein", "Position", "Einsatz", "SpielMinuten", "Error (Fehler vor Schuss/Gegentor)"])
...
for element in player_list:
    try:
        ...
        errorStr = element.findAll("td", {"class": "text-right"})[2].text
        error = int(errorStr)
        ...
        print(name)
        writer.writerow([name, verein, position, einsatz, spielminuten, error])
    except:
