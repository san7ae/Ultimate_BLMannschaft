...
page = "https://www.ligainsider.de/bundesliga/ranking-top-10/weisse-weste/"
pageTree = requests.get(page, headers=headers)
pageSoup = bs4.BeautifulSoup(pageTree.content, 'html.parser')

# Write to CSV data
outfile = open('cleanSheet.csv','w', encoding="utf-8", newline='')
writer = csv.writer(outfile, delimiter=" ")
writer.writerow(["Name", "Verein", "Position", "Einsatz", "SpielMinuten", "Clean Sheet"])
...
for element in player_list:
    try:
        ...
        cleanSheetStr = element.findAll("td", {"class": "text-right"})[2].text
        cleanSheet = int(cleanSheetStr)




        print(name)
        writer.writerow([name, verein, position, einsatz, spielminuten, cleanSheet])
    except:
